...
import { getAllTestimonials, createTestimonial, updateTestimonial, deleteTestimonial } from '@/lib/firebase-utils'
...
export default function TestimonialsPage() {
  ...
  const handleUpdateStatus = async (id: string, status: Testimonial['status']) => {
    try {
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const testimonialData = {
      ...
      status: (editingTestimonial?.status || 'approved') as Testimonial['status'],
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-3">
                    <h4 className="text-lg font-medium text-charcoal font-awesome-serif">
                      {testimonial.name}
                    </h4>
